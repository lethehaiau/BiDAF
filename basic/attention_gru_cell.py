    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with vs.variable_scope(scope or "attention_gru_cell"):
            with vs.variable_scope("gates"):  # Reset gate and update gate.
                ...
                print("inputs1: ", inputs.get_shape())
                inputs_ = tf.unpack(inputs, axis=1)
                #print("inputs2: ", inputs_.get_shape())
                g = tf.expand_dims(inputs_[self._num_units], -1)
                print("g: ", g.get_shape())
                inputs__ = []
                for i in range(0, self._num_units):
                    inputs__.append(tf.expand_dims(inputs_[i],-1))
                inputs = tf.concat(1, inputs__)
